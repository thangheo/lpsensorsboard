...
void read_accel_sensor_task(void *pvParameters){
    const uint8_t num_acc_channel =3;
    uint16_t accel_val[3]={0,0,0};
    for(;;){
        xSemaphoreTake(ADC1_Mutex, portMAX_DELAY);
        sensor_data_fusion_t *data = pvParameters;
        ...
        */
        ...
        for(int idx=0;idx < 10;idx++){
            ...
            for(uint8_t channel_idx=0;channel_idx <num_acc_channel;channel_idx++){        
                sConfig.Channel = channels[channel_idx];    
                if (HAL_ADC_ConfigChannel(&hadc, &sConfig) != HAL_OK)
                {
                    Error_Handler();
                ...
                {
                    /*##-6- Get the converted value of regular channel  ########################*/
                    accel_val[channel_idx] += HAL_ADC_GetValue(&hadc);
                }
                //sleep 2ms
                vTaskDelay( 10 / portTICK_PERIOD_MS);
            }   //channels scan loop
        ...
        accel_val[1] /=10;
        accel_val[2] /=10;
        //dot product of accleration
        data->accleration = sqrt(accel_val[0]*accel_val[0] + accel_val[1]*accel_val[1]  +
                            accel_val[2]*accel_val[2]);
...
void read_audio_data_task(void *pvParameters){
    for(;;){
       ...
       for(int idx=0;idx < 10;idx++){
        ...
        data->audio = adc_val/10; //just get the average amplitute of sound
